...
public sealed class PdfTools(IHttpClientFactory httpClientFactory, AgentArtifactStore artifactStore, IContentStore contentStore)
{
    static PdfTools()
    {
    ...
    public async Task<string> GeneratePdfAsync(
        ...
        [Description("Optional conditional formatting rules for content-based export (bold/italic only in PDF). Used only for tabular data.")] ConditionalRule[]? rules = null,
        ...
        [Description("Markdown content for narrative/free-form documents. Used only when no contentId is provided.")] string? content = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(contentId))
        {
            ...
            markdownContent = stored.ContentType is "table"
        ...
        var parsedMarkdown = ParsedMarkdownDocument.FromText(markdownContent);
        await parsedMarkdown.DownloadImages(httpClient: httpClientFactory.CreateClient());
        ...
        var bytes = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Markdown(parsedMarkdown);
            });
        }).GeneratePdf();
